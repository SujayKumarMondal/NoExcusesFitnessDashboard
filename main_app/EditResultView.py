...
class EditResultView(View):
    def get(self, request, *args, **kwargs):
        ...
        return render(request, "trainer_template/edit_member_result.html", context)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            try:
                ...
                # Validating
                result = MemberResult.objects.get(member=member, work_out_plan=work_out_plan)
                result.exam = exam
                result.test = test
                result.save()
                messages.success(request, "Result Updated")
                return redirect(reverse('edit_member_result'))
